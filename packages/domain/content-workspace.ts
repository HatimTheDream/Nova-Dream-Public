...
const id=z.string().min(1).max(100).regex(/^[a-zA-Z0-9:_-]+$/),rev=z.number().int().positive();
export const contentModes=['research','draft','improve','review'] as const;
export type ContentMode=typeof contentModes[number];
export const libraryItemSchema=z.object({id,name:z.string().trim().min(1).max(100),type:z.enum(['brand','template']),revision:rev,archived:z.boolean(),guidance:z.string().max(10000),brief:z.string().max(10000),body:z.string().max(100000),format:z.enum(['markdown','text']).optional(),assets:z.array(attachmentSchema).max(32).refine(a=>new Set(a.map(f=>f.id)).size===a.length).optional(),platform:z.string().trim().min(1).max(100),collection:z.string().max(100),tags:z.array(z.string().trim().min(1).max(50)).max(20),brandId:id.nullable()}).strict();
...
const base={requestId:z.uuid(),epoch:z.uuid()},source={contentId:id,expectedRevision:rev};
export const contentWorkspaceCommandSchema=z.discriminatedUnion('type',[
