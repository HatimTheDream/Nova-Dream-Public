...
export const companionTools = ['start_session', 'end_session', 'launch_app', 'bring_to_front', 'list_windows', 'get_window_state', 'click', 'press_key'] as const;
...
export type CompanionDevice = { id: string; name: string; platform: 'darwin' | 'win32' | 'linux'; linkedAt: number; lastSeenAt: number; revokedAt?: number; connected: boolean; enabledUntil: number; apps: string[]; tools?: z.infer<typeof companionToolSchema>[] };
