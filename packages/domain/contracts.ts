...
const id = /* @__PURE__ */ (() => z.string().min(1).max(100).regex(/^[a-zA-Z0-9:_-]+$/))();
const unique = <T>(items: T[]) => new Set(items).size === items.length;
export const appIconSchema = /* @__PURE__ */ (() => z.enum(['red', 'cream']))();
...
const date = /* @__PURE__ */ (() => z.string().refine(value => value === '' || (/^\d{4}-\d{2}-\d{2}$/.test(value) && Number.isFinite(Date.parse(value)) && new Date(value).toISOString().slice(0, 10) === value)))();
export const taskSchema = /* @__PURE__ */ (() => z.object({ title: z.string().trim().min(1).max(300), notes: z.string().max(10000), status: z.enum(['open', 'active', 'waiting', 'blocked', 'done', 'skipped']), planned: date, due: date,
