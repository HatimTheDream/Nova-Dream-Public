...
export const calendarRangeSchema = /* @__PURE__ */ (() => z.object({ from: localDate, to: localDate, timezone }).strict().refine(v => v.to > v.from && Date.parse(v.to) - Date.parse(v.from) <= 62 * 86400000, 'Choose a calendar window of at most 62 days.'))();
...
export const calendarSelectionSchema = /* @__PURE__ */ (() => z.object({ requestId: z.uuid(), epoch: z.uuid(), expectedRevision: z.number().int().nonnegative(), sourceIds: z.array(z.string().min(1).max(100)).max(50).refine(ids => new Set(ids).size === ids.length), showLocal: z.boolean(), showTasks: z.boolean() }).strict())();
export const calendarRefreshSchema = /* @__PURE__ */ (() => z.object({ requestId: z.uuid(), epoch: z.uuid(), range: calendarRangeSchema, sourceIds: z.array(z.string().min(1).max(100)).max(50).refine(ids => new Set(ids).size === ids.length) }).strict())();
