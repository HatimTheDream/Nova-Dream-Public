...
export function VersionList({ conversation, conversations, open }: { conversation: Conversation; conversations: Conversation[]; open: (conversation: Conversation) => void }) {
  const versions = relatedConversations(conversation, conversations);
  return <><span className="eyebrow">Conversation versions</span>{versions.map((item, i) => <button key={item.id} aria-current={item.id === conversation.id ? 'true' : undefined} disabled={item.deleted || item.state !== 'ready'} onClick={() => { if (item.id !== conversation.id) open(item); }}><span>{i === 0 ? 'Original' : item.forkSource?.purpose === 'edit' ? 'Edited message' : item.forkSource?.purpose === 'retry' ? 'Alternate response' : 'Branch'}<small>{item.title}{item.deleted ? ' · Deleted' : item.archived ? ' · Archive' : item.state !== 'ready' ? ' · Needs status check' : ''}</small></span>{item.id === conversation.id && <Check size={16}/>}</button>)}<p className="metadata">Each version keeps its own messages and draft.</p></>;
}
