...
export function useDictation(epoch: string, draftId: string, append: (text: string) => void) {
  ...
  const release = () => { acquiring.current?.(); browser.current?.close(); capturing.current = false; processor.current?.disconnect(); processor.current = undefined; media.current?.getTracks().forEach(t => t.stop()); media.current = undefined; void audio.current?.close().catch(() => undefined); audio.current = undefined; clearInterval(timer.current); };
  ...
  const keep = (a: DictationAttempt, expectedTicket = ticket.current) => {
    ...
    retire(a); attempt.current = a; if (a.route !== 'browser' || !browser.current) latest.current = a.text;
    saveLocal(key, { text: latest.current, attemptId: a.id });
    if (alive.current) setPreview(latest.current);
  };
  const end = async (a: DictationAttempt) => { const ended = await request<DictationAttempt>('assistant/dictation/end', { requestId: crypto.randomUUID(), epoch, attemptId: a.id }, undefined, 35000); retire(ended); return ended; };
  const cancel = async () => {
    ...
    release(); browser.current = undefined;
    if (alive.current) setPhase(known || pending ? 'finishing' : 'idle');
    try { const a = known ?? await pending; if (a) keep(await end(a), cancelledTicket); }
    ...
    finally { if (cancelledTicket === ticket.current) { starting.current = false; stopping.current = false; if (alive.current) setPhase('idle'); } }
  };
  const stop = async (insert = true) => {
    if (stopping.current) return; stopping.current = true; if (alive.current) setPhase('finishing');
    let ended = false;
    try { if (browser.current) await browser.current.finish(); draining.current = ticket.current; ticket.current++; release(); await chain.current; draining.current = undefined; if (failed.current) throw Error('Dictation was interrupted. Available text is kept.'); if (attempt.current) { const result = await end(attempt.current); ended = true; keep(result); if (result.state === 'failed' || result.error) throw Error(result.error ?? 'Dictation could not finish.'); } if (insert && alive.current) { if (latest.current.trim()) { appendRef.current(latest.current.trim()); localStorage.removeItem(key); latest.current = ''; setPreview(''); } else setError('No speech was detected. Try again and speak clearly into the microphone.'); } }
    catch (e) { if (alive.current) setError(e instanceof Error ? e.message : 'Dictation ended. Available text is kept.'); }
    finally { draining.current = undefined; const endingTicket = ++ticket.current; release(); if (!ended && attempt.current) await end(attempt.current).then(a => keep(a, endingTicket)).catch(() => undefined); browser.current = undefined; starting.current = false; stopping.current = false; if (alive.current) setPhase('idle'); }
  };
  const fail = (reason: unknown, expectedTicket = ticket.current) => {
    if (!current(expectedTicket) || failed.current) return;
    failed.current = true; draining.current = undefined; const endingTicket = ++ticket.current; release();
    ...
    if (attempt.current) void end(attempt.current).then(a => keep(a, endingTicket)).catch(() => undefined).finally(() => { if (endingTicket === ticket.current) { starting.current = false; browser.current = undefined; if (alive.current) setPhase('idle'); } });
    else starting.current = false;
  };
  const start = async () => {
    if (phase !== 'idle' || starting.current || stopping.current || latest.current.trim()) return;
    ...
    try {
      ...
      if (!current(capturedTicket) || stopping.current) { if (!stopping.current) await end(a); return; }
      if (a.state === 'listening' && a.route === 'browser') {
        ...
        let reading = false;
        timer.current = setInterval(() => { if (reading || !current(capturedTicket)) return; reading = true; void request<DictationAttempt>(`assistant/dictation/${a.id}`).then(next => { if (current(capturedTicket) && next.state !== 'listening') fail(Error(next.error ?? 'Dictation ended.'), capturedTicket); }).catch(e => fail(e, capturedTicket)).finally(() => { reading = false; }); }, 2000);
      ...
      node.onaudioprocess = e => {
        if (!capturing.current || !current(capturedTicket)) return;
      ...
      let reading = false;
      timer.current = setInterval(() => { if (reading || !capturing.current || !current(capturedTicket)) return; reading = true; void request<DictationAttempt>(`assistant/dictation/${a.id}`).then(next => { if (!current(capturedTicket)) return; keep(next, capturedTicket); if (next.state !== 'listening') fail(Error(next.error ?? 'Dictation ended. Use the available text below.'), capturedTicket); }).catch(e => fail(e, capturedTicket)).finally(() => { reading = false; }); }, 650);
    } catch (e) { fail(e, capturedTicket); }
  };
  useEffect(() => { alive.current = true; return () => { alive.current = false; draining.current = undefined; const endingTicket = ++ticket.current; release(); if (attempt.current) void end(attempt.current).then(a => keep(a, endingTicket)).catch(() => undefined); }; }, []);
  const discard = () => { latest.current = ''; setPreview(''); setError(''); localStorage.removeItem(key); };
  const useText = () => { if (latest.current.trim()) appendRef.current(latest.current.trim()); discard(); };
  return { phase, preview, error, start, stop, cancel, useText, discard };
}
