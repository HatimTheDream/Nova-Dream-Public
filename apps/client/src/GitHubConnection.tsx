...
export function GitHubConnection({epoch,onConnected}:{epoch:string;onConnected?:()=>void}){
  ...
  return <section className="github-connection">
    <div className="section-heading"><div><strong>GitHub</strong>{!state?.account&&<p className="metadata">{state?.message??'Checking GitHub connection…'}</p>}</div>{!state?.account&&!waiting&&<button type="button" disabled={busy||!state?.available} onClick={()=>void github.action('connect')}>Connect GitHub</button>}</div>
    {state?.account&&<details className="settings-account-row"><summary><span><strong>{state.account.login}</strong><span className="metadata">Connected</span></span><span className="settings-manage-label">Manage</span></summary><p className="metadata">{state.message}</p><button type="button" disabled={busy} onClick={()=>void github.action('disconnect')}>Disconnect</button></details>}
    ...
    {state&&!state.available&&<p className="metadata"><a href="https://cli.github.com/" target="_blank" rel="noreferrer">GitHub CLI setup</a></p>}
