...
function ActionCard({a,epoch,refresh}:{a:ModuleAction;epoch:string;refresh:()=>Promise<void>}){
 ...
 <details><summary>Review changes</summary><Value value={fields}/>{(review?.warnings??[]).map((v:string,i:number)=><p key={i}>{v}</p>)}{review?.attendees>0&&<p>Updates may notify {review.attendees} attendees.</p>}</details>
...
export function ModuleActionTray({conversationId,assignmentId,epoch,refreshWorkspace}:{conversationId?:string;assignmentId?:string;epoch:string;refreshWorkspace:()=>Promise<void>}){
 ...
 useEffect(()=>{let stopped=false,running=false,signature='';const controller=new AbortController();setItems([]);if(!conversationId&&!assignmentId)return;const read=async()=>{if(running)return;running=true;try{const next=await request<ModuleAction[]>('assistant/module-actions',assignmentId?{assignmentId}:{conversationId},controller.signal);if(!stopped){setItems(next);setError('');const changed=next.filter(a=>['applied','partial'].includes(a.state)).map(a=>a.id+':'+a.revision).join('|');if(changed!==signature){signature=changed;await refreshRef.current();}}}catch(e){if(!stopped)setError(e instanceof Error?e.message:'Workspace changes are unavailable.');}finally{running=false;}};void read();const timer=setInterval(()=>void read(),3000);return()=>{stopped=true;clearInterval(timer);controller.abort();};},[conversationId,assignmentId,epoch]);
 const refresh=async()=>{if(!conversationId&&!assignmentId)return;setItems(await request('assistant/module-actions',assignmentId?{assignmentId}:{conversationId}));await refreshWorkspace();};
