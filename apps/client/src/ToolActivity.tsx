...
export function StepsPill({ plan, operation }: { plan?: RunStep[]; operation?: AssistantOperation }) {
  ...
  useEffect(() => { if (!expanded || settled) return; setNow(Date.now()); const timer = setInterval(() => setNow(Date.now()), 1000); return () => clearInterval(timer); }, [expanded, settled]);
  useEffect(() => {
    if (!expanded) return;
  ...
  const current = activeIndex >= 0 ? activeIndex + 1 : Math.min(plan.length, complete + (settled || complete === plan.length ? 0 : 1));
  const interrupted = operation?.state === 'failed' || operation?.state === 'cancelled';
  const status = operation?.cancelRequested && !settled ? 'Stopping' : operation?.state === 'failed' ? 'Needs attention' : operation?.state === 'cancelled' ? 'Stopped' : operation?.state === 'unknown' ? 'Checking progress' : complete === plan.length ? 'Completed' : settled ? 'Reply finished' : 'Live';
  const start = Date.parse(operation?.createdAt ?? ''), end = settled ? Date.parse(operation!.updatedAt) : now;
  ...
  return <section ref={root} className="steps-pill" onPointerEnter={event => { if (event.pointerType === 'mouse') setHovered(true); }} onPointerLeave={event => { if (event.pointerType === 'mouse') setHovered(false); }} onBlur={event => { if (!event.currentTarget.contains(event.relatedTarget as Node | null)) close(); }} onKeyDown={event => { if (event.key === 'Escape') { event.preventDefault(); event.stopPropagation(); close(); trigger.current?.focus({ preventScroll: true }); } }}>
    {expanded && <div id={panelId} className="run-plan-popover" role="region" aria-label="Task steps">
      ...
      <ol className="run-steps">{plan.map(step => <li key={step.id} className={`run-step run-step--${step.status}`} aria-current={step.status === 'active' ? 'step' : undefined}><span className="run-step-indicator" aria-label={step.status === 'complete' ? 'Complete' : step.status === 'active' ? 'In progress' : 'Not started'}>{step.status === 'complete' ? <Check size={15}/> : <span className="run-step-ring"/>}</span><span><strong>{step.label}</strong>{step.detail && <small>{step.detail}</small>}</span></li>)}</ol>
    </div>}
    <button ref={trigger} className={`run-plan-pill${interrupted ? ' run-plan-pill--interrupted' : ''}`} type="button" aria-label={`Steps, ${current} of ${plan.length}, ${complete} complete`} title={`${complete} of ${plan.length} steps complete`} aria-expanded={expanded} aria-controls={expanded ? panelId : undefined} onClick={() => { if (pinned) close(); else setPinned(true); }}><span className="steps-pill-face">{interrupted ? <Interrupted size={13}/> : complete === plan.length ? <Check size={15}/> : settled ? <span className="run-step-ring"/> : <span className="run-pulse"/>}<strong>{current}/{plan.length}</strong></span></button>
  </section>;
}
export function ToolActivity({ operation }: { operation: AssistantOperation }) {
  ...
  const working = tools.filter(t => t.state === 'running').length;
  return <details className="tool-activity compact-activity"><summary><span className="steps-status">{working ? activityLabel(tools.find(t => t.state === 'running')!.name, true) : tools.length === 1 ? activityLabel(tools[0].name) : 'View activity'}</span></summary>{tools.length === 100 && <p>Showing the latest 100 tool calls.</p>}<ol>{tools.map(tool => <li key={tool.id}><details><summary><span>{tool.title ?? activityLabel(tool.name, tool.state === 'running')}</span><small>{tool.state === 'unknown' ? 'Outcome unconfirmed' : tool.state}</small></summary><p className="metadata">{tool.name}</p>{tool.output && <pre>{tool.output}</pre>}{tool.truncated && <p className="metadata">Showing the first part of this output. The full result remains in the native conversation.</p>}</details></li>)}</ol></details>;
}
