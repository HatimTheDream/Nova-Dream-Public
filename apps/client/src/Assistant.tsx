...
import { AssistantNavigation, AssistantSpaceSwitch } from './AssistantSpaceSwitch';
...
function Editor({ snapshot, journal, controller, voice, draftId, openSettings, newProject, editProject, initialPreview, refreshWorkspace, rail, contentActions, sourceRequest }: Props & { journal: Journal; controller: AssistantController; draftId: string; rail: AssistantRailState }) {
  ...
  const spaceSwitchDisabled = busy || !!journal.storageError || !!attachments.pending.length || dictation.phase !== 'idle' || !!dictation.preview;
  ...
  return <div className={`assistant-workspace ${sidePanel || activityOpen ? 'inspector-open' : ''}`}>
    ...
    <div className={`conversation ${reading ? 'writing-kept' : ''}`} inert={organizationOpen && !rail.desktop}>
      <AssistantNavigation space={controller.space} change={switchSpace} disabled={spaceSwitchDisabled} teamOpen={sidePanel?.kind === 'team'} openTeam={openTeam}/>
