...
function Editor({ snapshot, journal, controller, voice, appIcon, draftId, openSettings, newProject, editProject, initialPreview, refreshWorkspace, rail, contentActions, sourceRequest }: Props & { journal: Journal; controller: AssistantController; draftId: string; rail: AssistantRailState }) {
  ...
  useEffect(() => {
    ...
    const fit = () => { field.style.height = 'auto'; field.style.height = `${Math.min(180, Math.max(42, field.scrollHeight))}px`; setCompactComposer(field.clientWidth < 350); };
  ...
  const renderMessage = (message: ConversationMessage) => {
    ...
    return <article aria-label={`${message.role} message`} className={`chat-message message-${message.role} ${progressing ? 'message-progress' : ''} ${message.toolInfo && (message.role === 'tool' || !message.text.trim()) ? 'message-tool-compact' : ''}`} id={`message-${message.novaId ?? message.id}`} key={message.novaId ?? `${message.role}:${message.id}`}>
      ...
      <HistoryTool message={message}/><SavedMessageText message={message}/>
      ...
      {(message.role === 'user' || message.role === 'assistant' && !!message.text.trim() && !progressing) && <MessageActions
        ...
        reading={readingAloud.messageId === (message.novaId ?? message.id) && ['speaking', 'paused'].includes(readingAloud.phase)} readAloud={() => readingAloud.messageId === (message.novaId ?? message.id) ? aloud.stop() : aloud.start(message.novaId ?? message.id, message.authoredText ?? message.text)}
  ...
  const belongsToActiveReply = (message: ConversationMessage) => !!active && resolveMessageActions(message, conversation!, controller.history, operations, []).operation?.id === active.id;
  const renderTranscriptMessage = (message: TranscriptMessage) => message.voiceParts || message.pendingVoice ? <VoiceMessage message={message} renderPart={renderMessage}/> : renderMessage(message);
...
<div className="conversation-scroll-region"><div className="conversation-reading" ref={setReadingParent} tabIndex={0} aria-label="Conversation history">
        ...
        {preview ? <article className="draft-preview"><span className="eyebrow">Saved draft · read only</span><h2>{preview.value.title}</h2><p className="preserve-lines">{preview.value.text || 'This draft contains attachments.'}</p>{preview.value.attachments.map(a => <a className="source-link" key={a.id} href={`/api/attachments/${a.id}`}><File size={16}/>{a.name}</a>)}<button onClick={() => append(preview.value)}>Append to my draft</button><button className="quiet" onClick={() => setPreview(null)}>Back to my draft</button></article> : conversation ? <div className="chat-transcript" aria-label="Conversation messages">
          ...
          {readingParent && controller.history && <VirtualTranscript ref={transcript} key={conversation.id} parent={readingParent} scroll={scroll} history={controller.history} cacheKey={transcriptCacheKey(snapshot.epoch, conversation)} positionKey={transcriptPositionKey(snapshot.epoch, snapshot.deviceId, conversation)} messages={groupVoiceMessages(voiceMessages.filter(m => !(belongsToActiveReply(m) && (!!m.toolInfo && !m.text.trim() || m.role === 'tool' || m.role === 'assistant' && m.text === active?.text)) && !(belongsToActiveReply(m) && m.toolInfo?.id && active?.tools?.some(tool => tool.id === m.toolInfo?.id))))} render={renderTranscriptMessage} footer={<>
{controller.history && !voiceMessages.length && !controller.loading && !controller.historyError && !active && <Empty title={conversation.state === 'ready' ? 'A clear place to think.' : conversation.state === 'failed' ? 'Conversation setup was rejected.' : 'Waiting for OpenClaw confirmation.'}>{conversation.state === 'ready' ? 'Your first message will use this conversation and its selected Project.' : conversation.error ?? 'Use Check status to find the original native session. It will not be created again automatically.'}</Empty>}
          {active && !controller.history?.hasNewer && <article className="chat-message message-assistant live-message"><div className="working-status" role="status"><span className={active.state === 'unknown' ? 'working-dot is-unconfirmed' : 'working-dot'} aria-hidden="true"/>{active.state === 'unknown' ? 'Progress not confirmed' : active.cancelRequested ? 'Stopping…' : active.state === 'running' ? 'Working…' : 'Starting…'}</div><ToolActivity operation={active}/>{active.text && <ReplyText text={active.text} role="assistant" streaming={active.state !== 'unknown'}/>}{active.error && <p>{active.error}</p>}{active.state === 'unknown' && <button className="text-button" onClick={() => void controller.checkStatus()}>Check status</button>}</article>}
      ...
      {conversation?.archived ? <div className="archive-footer"><Archive size={19}/><span>{conversation.deleted ? 'Deleted conversation. Your unsent draft is kept.' : 'This archive is read only. Your unsent draft is kept.'}</span><button onClick={() => void controller.edit(conversation, conversation.deleted ? { deleted: false } : { archived: false }).catch(keepError)}>{conversation.deleted ? 'Restore to Archive' : 'Restore conversation'}</button></div> : <div className="composer-region">{!reading && <ModuleActionTray conversationId={conversation?.id} epoch={snapshot.epoch} refreshWorkspace={refreshWorkspace}/>} {!reading && <ApprovalTray controller={controller} conversationId={conversation?.id} epoch={snapshot.epoch} working={!!active}/>} {!reading && readingAloud.phase !== 'idle' && <div className="read-aloud-controls" role="group" aria-label="Read aloud controls"><span>{readingAloud.error ?? (readingAloud.phase === 'paused' ? 'Reading paused' : 'Reading reply')}</span>{readingAloud.phase !== 'error' && <button className="text-button" onClick={aloud.toggle}>{readingAloud.phase === 'paused' ? 'Resume reading' : 'Pause reading'}</button>}<button className="icon-button" aria-label={readingAloud.phase === 'error' ? 'Dismiss read aloud notice' : 'Stop reading'} onClick={aloud.stop}><X size={17}/></button></div>}{!reading && <VoicePanel appIcon={appIcon} conversationId={conversation?.id} controller={voice} openConversation={id => { setReading(null); setPreview(null); controller.select(id); }}/> }
        {conversation && (queueExpanded || controller.queue?.some(q => q.conversationId === conversation.id && q.state === 'paused')) && <details className="composer-queue" open={queueExpanded} onToggle={e => setQueueExpanded(e.currentTarget.open)}><summary>{messageQueueSummary((controller.queue ?? []).filter(q => q.conversationId === conversation.id))}</summary><MessageQueue controller={controller} conversationId={conversation.id} epoch={snapshot.epoch} blocked={!!active || callingHere || !!conversation.pendingSettings} copy={append}/></details>}
