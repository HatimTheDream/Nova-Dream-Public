...
import { pendingVoiceTurns } from './voice-transcript';
...
type Props = { contentActions: ContentOutputActions; sourceRequest?: { target: BrowseTarget; nonce: string } | null; snapshot: Snapshot; controller: AssistantController; voice: VoiceController; legacyJournal: Journal; refreshWorkspace: () => Promise<void>; openSettings: () => void; newProject: () => void; editProject: (id: string) => void; initialPreview?: Entity<Draft> };
...
function Editor({ snapshot, journal, controller, voice, draftId, openSettings, newProject, editProject, initialPreview, refreshWorkspace, rail, contentActions, sourceRequest }: Props & { journal: Journal; controller: AssistantController; draftId: string; rail: AssistantRailState }) {
  ...
  const liveVoiceTurns = pendingVoiceTurns(voiceState, conversation, controller.history);
  const voiceTranscript = liveVoiceTurns.length ? <div className="live-voice-transcript" aria-label="Live voice messages">{liveVoiceTurns.map(turn => <article key={turn.turnId} className={`chat-message message-${turn.role} ${turn.role === 'assistant' ? 'live-message' : ''}`} aria-label={`${turn.role} voice message`}><ReplyText text={turn.text} role={turn.role} streaming={!turn.final}/></article>)}</div> : null;
  ...
  const send = async () => {
    ...
    if (busy || active || journal.dirty || journal.saving || journal.conflict || attachments.pending.length) return;
  ...
  const steer = async () => {
    if (!conversation || !active || busy || journal.dirty || journal.saving || journal.conflict || attachments.pending.length || draft.attachments.length) return;
  ...
  const enqueue = async () => {
    if (sourceCount > 10) { setContextDialog({ kind: 'sources' }); return; }
    if (!conversation || busy || journal.dirty || journal.saving || journal.conflict || attachments.pending.length) return;
  ...
  const startAndSend = async () => {
    if (busy || !ready || journal.dirty || journal.saving || journal.conflict || attachments.pending.length) return;
  ...
  const startVoice = async () => {
    ...
    if (draft.attachments.length || attachments.pending.length) { setNotice('Send or remove the draft files before starting voice.'); return; }
    setBusy(true); setNotice('');
    try {
  ...
  return <div className={`assistant-workspace ${sidePanel || activityOpen ? 'inspector-open' : ''}`}>
    <AssistantOrganizationRailFrame spaceSwitch={<AssistantSpaceSwitch value={controller.space} disabled={busy || !!journal.storageError || !!attachments.pending.length || dictation.phase !== 'idle' || !!dictation.preview} change={next => { if (next !== controller.space) { void journal.flush(); controller.switchSpace(next); } }}/>} newDraftLabel={controller.space === 'work' ? 'New work' : 'New chat'} open={organizationOpen} width={rail.width} setWidth={rail.setWidth} onToggle={toggleOrganization} onNewDraft={openNewDraft} onSearch={() => setSearchOpen(value => !value)}>
...
<div className="conversation-scroll-region"><div className="conversation-reading" ref={setReadingParent} tabIndex={0} aria-label="Conversation history">
        ...
        {preview ? <article className="draft-preview"><span className="eyebrow">Saved draft · read only</span><h2>{preview.value.title}</h2><p className="preserve-lines">{preview.value.text || 'This draft contains attachments.'}</p>{preview.value.attachments.map(a => <a className="source-link" key={a.id} href={`/api/attachments/${a.id}`}><File size={16}/>{a.name}</a>)}<button onClick={() => append(preview.value)}>Append to my draft</button><button className="quiet" onClick={() => setPreview(null)}>Back to my draft</button></article> : conversation ? <div className="chat-transcript" aria-label="Conversation messages">
          {controller.loading && !controller.history && <p className="metadata" role="status">Loading your conversation…</p>}
...
{controller.history && !liveVoiceTurns.length && !controller.history.messages.length && !controller.loading && !controller.historyError && !active && <Empty title={conversation.state === 'ready' ? 'A clear place to think.' : conversation.state === 'failed' ? 'Conversation setup was rejected.' : 'Waiting for OpenClaw confirmation.'}>{conversation.state === 'ready' ? 'Your first message will use this conversation and its selected Project.' : conversation.error ?? 'Use Check status to find the original native session. It will not be created again automatically.'}</Empty>}
        ...
        </div> : <div className="assistant-welcome"><img src="/mascot/lynx-focus.webp" alt="Nova at rest"/><h1>{controller.space === 'work' ? 'What would you like to get done?' : 'What’s on your mind?'}</h1><p>{controller.space === 'work' ? 'Bring a task. We’ll work through it together.' : 'Bring a question, a plan, or an unfinished thought.'}</p><div className="suggestions">{controller.space==='work'?<><button onClick={newProject}>Open repository</button><button onClick={()=>setSidePanel({kind:'team'})}>Team work</button><button onClick={()=>setSidePanel({kind:'browser'})}>Host browser</button></>:<><button onClick={() => journal.change(v => ({ ...v, text: `${v.text}${v.text ? '\n\n' : ''}Help me plan my day. The things I want to move forward are: ` }))}>Plan my day</button><button onClick={() => journal.change(v => ({ ...v, text: `${v.text}${v.text ? '\n\n' : ''}Help me think through this idea: ` }))}>Think through an idea</button></>}</div></div>}
      </div>
    ...
    {(reading || conversation?.archived) && <VoicePanel floating controller={voice} openConversation={id => { setReading(null); controller.select(id); }}/> }
    {memorySeed && <Suspense fallback={<p role="status">Opening memory…</p>}><MemoryEditor key={`${snapshot.epoch}:${memorySeed.source.messageId}`} snapshot={snapshot} seed={memorySeed} refresh={controller.refresh} close={() => setMemorySeed(null)}/></Suspense>}
    {continueSaved && conversation && <Suspense fallback={<p role="status">Opening saved transcript…</p>}><ContinueSavedConversation conversationId={conversation.id} snapshot={snapshot} controller={controller} refreshWorkspace={refreshWorkspace} close={() => setContinueSaved(false)}/></Suspense>}
    ...
    {!reading && workspace.opened && <AssistantSidePanel tabs={workspace.tabs} active={workspace.active} visible={workspace.visible} expanded={workspace.expanded} select={id => workspace.dispatch({ type: 'select', id })} closeTab={id => workspace.dispatch({ type: 'close', id })} addTab={() => setSidePanel({ kind: 'home' })} expand={() => workspace.dispatch({ type: 'expand', expanded: !workspace.expanded })} close={closePanel}>
      {workspace.tabs.map(tab => { const view = tab.view; const output = view.kind === 'file' ? conversationFiles.find(item => item.id === view.outputId && item.version === view.outputVersion) : undefined; return <section key={tab.id} className="workspace-tab-content" role="tabpanel" id={`workspace-view-${tab.id}`} aria-labelledby={`workspace-tab-${tab.id}`} hidden={workspace.active !== tab.id}>
        {view.kind === 'home' && <div className="assistant-workspace-launcher">{controller.space === "work" && <button onClick={()=>setSidePanel({kind:"team"})}><Folder size={20}/>Team work</button>}<button onClick={()=>setSidePanel({kind:"browser"})}><Device size={20}/>Browser</button>{openChanges && <button onClick={openChanges}><List size={20}/>Review</button>}{hasLiveView && <button onClick={() => setSidePanel({ kind: 'live' })}><Device size={20}/>Live tool view</button>}<button onClick={openFiles}><Folder size={20}/>Files</button></div>}
        {view.kind === 'files' && <ConversationFiles outputs={conversationFiles} sourceFiles={sourceFiles} openFile={openFile} addFiles={addFiles}/>}
        {view.kind === "team" && <Suspense fallback={<p role="status">Opening team work…</p>}><TeamWorkPanel snapshot={snapshot} projectId={project?.id} openConversation={id=>controller.select(id,"work")} active={workspace.visible && workspace.active===tab.id}/></Suspense>}
        {view.kind === "browser" && <Suspense fallback={<p role="status">Opening browser…</p>}><HostBrowserPanel epoch={snapshot.epoch} active={workspace.visible && workspace.active===tab.id}/></Suspense>}
        {view.kind === 'changes' && conversation && <Suspense fallback={<p role="status">Opening changes…</p>}><WorkProjectChanges conversation={conversation} epoch={snapshot.epoch}/></Suspense>}
        {view.kind === 'live' && <div ref={setActivityContainer} className="workspace-live-container"/>}
        {view.kind === 'file' && <><div className="workspace-file-toolbar"><span title={view.file.name}>{view.file.name}</span><a className="icon-button output-download" aria-label="Download file" title="Download file" href={`/api/attachments/${view.file.id}`}><Download size={18}/></a></div><Suspense fallback={<p role="status">Opening file…</p>}><AssistantFilePreview file={view.file}/></Suspense>{output && <div className="button-row"><button disabled={!!active || !!conversation?.archived} onClick={() => void refine(output)}>Refine</button><UseOutputInContent output={output} {...contentActions}/></div>}</>}
      </section>; })}
    </AssistantSidePanel>}
    {contextDialog && <Dialog title={contextDialog.kind === 'sources' ? contextDialog.operationId ? 'Sources used for this reply' : 'Message context' : contextDialog.kind === 'memory' ? 'Saved memories' : contextDialog.kind === 'history' ? 'Run history' : 'Technical details'} close={() => setContextDialog(null)}>
      ...
      {contextDialog.kind === 'memory' && <Suspense fallback={<p role="status">Opening memories…</p>}><MemoryPanel memory={controller.memory} snapshot={snapshot} refresh={controller.refresh} openSource={target => { setContextDialog(null); setReading(target); }}/></Suspense>}
