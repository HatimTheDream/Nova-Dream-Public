...
export function PhoneSettings({ epoch, deviceId, access }: { epoch: string; deviceId: string; access?: AccessContext }) {
  ...
  return <section className="card settings-card phone-settings"><h2><Device size={22}/> {access?.surface === 'web' ? 'Additional paired devices' : 'Private phone access'}</h2><p>{access?.surface === 'web' ? 'This web address works on your phone. Pair other devices without host settings.' : 'Use the same Inbox, Calendar, Tasks and Assistant on your phone.'}</p>
    {recoveryPaused && <p className="notice">Review connection setup in Backup & recovery before enabling phone access for this copy.</p>}<div className="setting-row"><div><strong>{state?.enabled ? 'Phone access enabled' : 'Phone access off'}</strong><p role="status">{state?.route.message ?? 'Checking phone settings…'}</p></div><button disabled={recoveryPaused || busy || !!pending || !state} onClick={() => void send(state?.enabled ? 'disable' : 'enable')}>{state?.enabled ? 'Turn off' : 'Enable'}</button></div>
    ...
    <details className="phone-help"><summary>Phone not connecting?</summary><ol><li>Open Tailscale on this computer and your phone. Both must be connected to the same private network.</li><li>If phone access is unavailable here, use Reconnect Tailscale. If Tailscale asks you to sign in, finish that in its app, then reconnect here.</li><li>Scan the QR code or open the private address in your phone's browser. Create a fresh pairing code if access was removed or the old code expired.</li></ol><p>Turning off Tailscale disconnects the phone. Your saved work stays in Nova.</p></details>
