...
function OptionsPicker({ props, filters, close }: { props: Props; filters: Filter[]; close(): void }) {
  ...
  return <div className="task-filter-picker" ref={root}>
    ...
    </> : current ? current.choices.map(choice => <button type="button" role="menuitemradio" aria-checked={choice.value === current.value} key={choice.value} onClick={() => { current.choose(choice.value); close(); }}><span className={current.id === 'project' ? 'preserve-case' : undefined}>{choice.label}</span>{choice.value === current.value && <Check size={16}/>}</button>) : <>
...
export function TaskFilters(props: Props) {
  ...
  return <>
    ...
    {(active.length > 0 || props.trash || props.sort !== 'planned' || props.selecting) && <div className="task-active-filters" aria-label="Current list choices">
      {active.map(filter => { const label = `${filter.label}: ${filter.choices.find(choice => choice.value === filter.value)?.label ?? 'Unavailable'}`; return <button type="button" key={filter.id} aria-label={`Remove ${label} filter`} title={`Remove ${label}`} onClick={() => filter.choose('all')}><span className={filter.id === 'project' ? 'preserve-case' : undefined}>{label}</span><X size={13}/></button>; })}
