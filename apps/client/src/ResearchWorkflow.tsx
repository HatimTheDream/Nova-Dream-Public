...
export function ResearchWorkflow({ item: supplied, operations, epoch, ready, connected, connectionGeneration, questions, readOnly, refresh, stop, update }: {
  ...
  const hasActivity = operation?.tools?.some(tool => !['progress_card', 'update_plan'].includes(tool.name.split(/__|\./).at(-1) ?? tool.name));
  const progressLabel = live ? `${status}; remaining work is not yet known` : status;
  return <section className="research-workflow" aria-label="Research">
    ...
    {item.approval && <div className="research-progress-region">
      ...
      <div className="research-progress-row">
        <div className={`research-progress${live ? ' research-progress--running' : ''}`} role="progressbar" aria-label="Research activity" aria-valuetext={progressLabel}/>
