...
export function ResearchWorkflow({ item: supplied, operations, epoch, ready, readOnly, refresh, stop, update }: {
  ...
  return <section className="research-workflow" aria-label="Research">
    <header><h3>{proposal?.title ?? 'Preparing your research plan'}</h3>{live && update && <button className="research-control research-update" disabled={!actionable || !operation.nativeRunId} onClick={() => update(operation.id)}>Update</button>}</header>
    ...
    {item.approval && <div className="research-progress-region">
      {running && <p className="research-status" role="status">{operation.cancelRequested ? 'Stopping…' : 'Researching…'}</p>}
      <div className="research-progress-row">
        <div className={`research-progress${!measured && live ? ' research-progress--running' : ''}`} role="progressbar" aria-label="Research progress" aria-valuemin={0} aria-valuemax={measured ? measuredSteps.length : undefined} aria-valuenow={measured ? completedSteps : undefined} aria-valuetext={measured ? progressLabel : live ? 'Researching; step progress not yet reported' : 'Progress unconfirmed'}>{measured && <span className="research-progress-fill" style={{ width: `${completedSteps / measuredSteps.length * 100}%` }}/>}</div>
