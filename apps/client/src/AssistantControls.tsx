...
import { effortPreference, responseEffortLevels } from '../../../packages/domain/auto-effort';
...
export const effortLabel = (value: string | null | undefined) => { const preference = effortPreference(value); return ({ xhigh: 'Extra high' }[preference] ?? preference[0].toUpperCase() + preference.slice(1)); };
...
export function ResponseControls({ value, models, modelStatus, retryModels, blocked, save }: { value: ResponsePreferences; models: AssistantModel[]; modelStatus: 'loading' | 'ready' | 'error' | 'offline'; retryModels: () => Promise<void>; blocked: boolean; save: (value: ResponsePreferences) => Promise<unknown> }) {
  ...
  const model = responseModel(models, value.model), levels = responseEffortLevels(model?.reasoning), thinking = effortPreference(value.thinking);
  ...
  const proposed = preview?.key === key ? preview : undefined, level = proposed?.index ?? Math.max(0, levels.indexOf(thinking));
  const shown = proposed ? levels[level] : thinking, applying = useRef(false);
  ...
  return <div className="response-picker">
    {selectingModel ? <><button className="text-button" onClick={() => setSelectingModel(false)}><ArrowLeft size={17}/>Response</button><span className="menu-heading">Select model</span><div className="response-model-list" role="group" aria-label="Select model">
      <button disabled={unavailable} aria-pressed={!value.model} onClick={() => { void change({ model: null, thinking: 'auto' }); setSelectingModel(false); }}><span>Default<small>{responseModel(models, null)?.name ?? 'Configured model'}</small></span>{!value.model && <Check size={17}/>}</button>
      {models.map(m => <button key={m.id} disabled={unavailable || !m.available} aria-pressed={m.id === value.model} onClick={() => { void change({ model: m.id, thinking: 'auto' }); setSelectingModel(false); }}><span>{m.name}{!m.available && <small>Unavailable</small>}</span>{m.id === value.model && <Check size={17}/>}</button>)}
    </div>{modelStatus === 'ready' && !models.some(m => m.available) && <p className="metadata">No models are available for this connection yet.</p>}</> : <>
      <div className="effort-heading"><button className={`response-speed ${value.fastMode === true ? 'enabled' : ''}`} aria-label={`Speed: ${value.fastMode === true ? 'Fast' : value.fastMode === false ? 'Standard' : value.fastMode === 'auto' ? 'Automatic' : 'Default'}`} aria-pressed={value.fastMode === true} title={value.fastMode === true ? 'Use standard speed' : 'Use fast speed'} disabled={unavailable} onClick={() => void change({ fastMode: value.fastMode === true ? false : true })}><Zap size={18}/></button><strong>{effortLabel(shown)}</strong><button aria-label="Reset response settings" title="Reset effort and speed" disabled={unavailable} onClick={() => void change({ thinking: 'auto', fastMode: null })}><Reset size={18}/></button></div>
      ...
      {modelStatus === 'ready' && levels.length < 2 && <p className="metadata effort-unavailable">{model ? 'This model does not expose effort settings.' : 'Choose a model to set effort.'}</p>}<div className="effort-scale"><span>{levels.length > 1 ? 'Auto' : ''}</span><span>{levels.length > 1 ? effortLabel(levels.at(-1)) : ''}</span></div>
    </>}
