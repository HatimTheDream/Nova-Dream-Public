...
export function ApprovalTray({ controller, conversationId, epoch, working }: { controller: AssistantController; conversationId?: string; epoch: string; working: boolean }) {
  ...
  const elsewhere = [...new Set([...pending, ...visibleQuestions.filter(q => !confirmedQuestion(q))].filter(item => item.conversationId !== conversationId).map(item => item.conversationId))];
  ...
  if (!here.length && questionHere.length === 1 && !messages.length && single.availability !== 'missing' && !single.action && single.snapshot.questions.every(question => !question.isSecret)) return <div className="approval-tray request-tray compact-question-tray"><QuestionCard key={single.id} item={single} epoch={epoch} ready={questions?.state === 'ready'} refresh={controller.refresh} compact/>{elsewhere.map(id => <button type="button" className="request-secondary" key={id} onClick={() => controller.select(id)}>Review · <span className="preserve-case">{controller.conversations.find(c => c.id === id)?.title ?? 'Conversation'}</span></button>)}</div>;
  const title = count > 1 ? `${count} requests need your attention` : here.length ? 'Approval needed' : questionHere.length ? questionHere[0].availability === 'missing' || questionHere[0].action ? 'Review request status' : 'Nova needs your answer' : elsewhere.length ? 'Requests in other chats' : 'Review connection';
  return <details className="approval-tray request-tray" open={count > 0}><summary>{title}</summary><div className="approval-tray-content">{messages.map(message => <p className="request-context" role="status" key={message}>{message}</p>)}{questionHere.map(item => <QuestionCard key={item.id} item={item} epoch={epoch} ready={questions?.state === 'ready'} refresh={controller.refresh}/>)}{here.map(item => <ApprovalCard key={item.id} item={item} epoch={epoch} ready={approvals?.state === 'ready'} refresh={controller.refresh}/>)}{elsewhere.map(id => <button type="button" className="request-secondary" key={id} onClick={() => controller.select(id)}>Review · <span className="preserve-case">{controller.conversations.find(c => c.id === id)?.title ?? 'Conversation'}</span></button>)}</div></details>;
}
