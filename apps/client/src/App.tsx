import { resetHomeWidgets } from '../../../packages/domain/home-widgets';
import { captureLayoutReset, undoLayoutReset, type LayoutResetUndo } from './layout-reset';
...
import { retainedWindowId, useRetained, useWorkspace } from './useWorkspace';
...
import { AppIconPicker, NovaAppMark, useAppIcon } from './AppIcon';
import { readCachedAppIcon, startupAppIcon, type StartupIconJournal } from './app-icon';
...
function Workspace({ snapshot, online, error, refresh, reconnect, access, updateRequired }: ReturnType<typeof useWorkspace> & { snapshot: Snapshot }) {
  ...
  const navMenuTrigger = useRef<HTMLElement | null>(null);
  const navigationKey = `e3:navigation-visible:${snapshot.deviceId}`;
  const [desktopNavigation, setDesktopNavigation] = useState(() => readLocal<boolean>(navigationKey) !== false);
  const [narrowNavigation, setNarrowNavigation] = useState(() => matchMedia('(max-width: 650px)').matches);
  const [navigationDrawer, setNavigationDrawer] = useState(false);
  const navigationRef = useRef<HTMLElement>(null), navigationToggle = useRef<HTMLButtonElement>(null);
  const navigationVisible = narrowNavigation ? navigationDrawer : desktopNavigation;
  const closeNavigation = () => { setNavigationDrawer(false); setNavMenu(null); };
  const drawerWasOpen = useRef(false);
  const toggleNavigation = () => { if (narrowNavigation) setNavigationDrawer(value => !value); else setDesktopNavigation(value => { saveLocal(navigationKey, !value); return !value; }); setNavMenu(null); };
  useEffect(() => {
    const media = matchMedia('(max-width: 650px)');
    const update = () => { setNarrowNavigation(media.matches); setNavigationDrawer(false); setNavMenu(null); };
    media.addEventListener('change', update); return () => media.removeEventListener('change', update);
  }, []);
  useEffect(() => {
    if (narrowNavigation && navigationDrawer) navigationRef.current?.querySelector<HTMLElement>('[aria-current="page"],button')?.focus();
    else if (drawerWasOpen.current) navigationToggle.current?.focus();
    drawerWasOpen.current = narrowNavigation && navigationDrawer;
  }, [narrowNavigation, navigationDrawer]);
  ...
  const [undoLayout, setUndoLayout] = useState<LayoutResetUndo | null>(null);
  ...
  const appIcon = useAppIcon(currentLayout.appIcon ?? snapshot.layout.value.appIcon);
  const nav = useReorder(currentLayout.nav, order => layout.change(value => ({ ...value, nav: order })), 'navigation', { motion: true, touchHold: true });
  useEffect(() => { if (nav.dragging) setNavMenu(null); }, [nav.dragging]);
  const open = (next: Route) => { if (next !== route && !mayLeaveAssistantDraft()) return false; setRoute(next); setNavMenu(null); if (narrowNavigation && navigationDrawer) closeNavigation(); if (next !== 'settings') setConnectionOrigin(null); return true; };
  ...
  const openReminder = (item: WorkspaceReminder) => {
    ...
    else { const task = snapshot.tasks.find(task => task.id === item.taskId); if (task) { setRemindersOpen(false); if (open('tasks')) editTask(task); } }
  };
  const notifications = useReminders(snapshot, online && !access?.recovery && !access?.recoveryLocal, refresh, openReminder, appIcon);
  ...
  return <div className="app-shell"><nav ref={navigationRef} id="main-navigation" className="primary-rail" role={narrowNavigation ? 'dialog' : undefined} aria-modal={narrowNavigation && navigationDrawer || undefined} onKeyDown={event => {
      if (!narrowNavigation || !navigationDrawer || event.defaultPrevented) return;
      if (event.key === 'Escape' && !navMenu) { event.preventDefault(); closeNavigation(); }
    ...
    }} data-reorder-host aria-label="Main navigation" hidden={!navigationVisible} inert={!navigationVisible}><div className="rail-drawer-heading"><strong>Navigation</strong><button className="icon-button" aria-label="Close main navigation" onClick={closeNavigation}><X size={20}/></button></div><button className="brand" aria-label="Nova Dream Home" onClick={() => open('home')}><NovaAppMark choice={appIcon}/></button><div className="rail-modules" ref={nav.containerRef} data-reorder-scroll>{nav.order.map(id => { const item = modules[id]; const Icon = item.icon; return <div key={id} className="nav-item-wrap" data-reorder-group="navigation" data-reorder-item={id}><button className={`nav-item ${item.color} ${route === id ? 'active' : ''} ${nav.dragging === id ? 'dragging' : ''}`} aria-label={item.label} aria-description="Hold or drag to move. Alt and Up or Down also changes position." aria-current={route === id ? 'page' : undefined} title={item.label} {...nav.bind(id)} onClick={() => { if (!nav.suppressClick.current) open(id); }} onContextMenu={event => { event.preventDefault(); if (!nav.dragging && !nav.suppressClick.current) { navMenuTrigger.current = event.currentTarget; setNavMenu(id); } }} onKeyDown={event => { if (event.altKey && ['ArrowUp', 'ArrowDown'].includes(event.key)) { event.preventDefault(); nav.move(id, nav.order.indexOf(id) + (event.key === 'ArrowUp' ? -1 : 1)); } else if (event.key === 'ContextMenu' || (event.shiftKey && event.key === 'F10')) { event.preventDefault(); navMenuTrigger.current = event.currentTarget; setNavMenu(id); } }}><span className="nav-icon"><Icon size={27}/></span><span className="nav-drawer-label">{item.label}</span></button></div>; })}</div><div className="rail-footer"><button className={`nav-item neutral ${route === 'settings' ? 'active' : ''}`} aria-label="Settings" title="Settings" aria-current={route === 'settings' ? 'page' : undefined} onClick={() => { setConnectionOrigin(null); open('settings'); }}><span className="nav-icon"><Settings size={26}/></span><span className="nav-drawer-label">Settings</span></button></div>{navMenu && <NavigationOptions key={navMenu} label={modules[navMenu].label} index={nav.order.indexOf(navMenu)} count={nav.order.length} move={index => nav.move(navMenu, index)} close={() => setNavMenu(null)} trigger={navMenuTrigger.current}/>}</nav>
    {narrowNavigation && navigationDrawer && <div className="main-navigation-scrim" aria-hidden="true" onClick={closeNavigation}/>}
    <div className="workspace" data-route={route} inert={narrowNavigation && navigationDrawer}><header className="topbar"><div className="topbar-leading"><button ref={navigationToggle} className="icon-button navigation-toggle" aria-label={navigationVisible ? "Hide main navigation" : "Show main navigation"} title={navigationVisible ? "Hide main navigation" : "Show main navigation"} aria-expanded={navigationVisible} aria-controls="main-navigation" onClick={toggleNavigation}>{navigationVisible ? <PanelLeft size={20}/> : <Menu size={20}/>}</button><div className="breadcrumb"><span>Nova Dream</span><span className="slash">/</span><strong>{title}</strong></div></div><div className="topbar-actions"><span className="build-tag">v{__E3_VERSION__}</span><button className="icon-button reminder-bell" aria-label={`Reminders${reminderCount ? ` · ${reminderCount} waiting` : ''}`} title="Reminders" onClick={() => setRemindersOpen(true)}><Bell size={20}/>{reminderCount > 0 && <span className="reminder-dot"/>}</button><button className="icon-button" aria-label="Search saved work" title="Search saved work" onClick={() => setSearch('')}><Search size={20}/></button><button className="icon-button" aria-label="Toggle color theme" title="Toggle color theme" onClick={() => layout.change(value => ({ ...value, theme: document.documentElement.dataset.theme === 'dark' ? 'light' : 'dark' }))}>{document.documentElement.dataset.theme === 'dark' ? <Sun size={20}/> : <Moon size={20}/>}</button></div></header>
      ...
      {route === 'settings' && <Suspense fallback={<ModuleLoading module="Settings"/>}><SettingsPage appIcon={appIcon} selected={settingsTab} select={setSettingsTab} snapshot={snapshot} online={online} access={access} openAssistant={() => open('assistant')} returnTo={connectionOrigin ? { label: modules[connectionOrigin].label, open: () => { open(connectionOrigin); setConnectionOrigin(null); } } : undefined} general={<><section className="card settings-card"><h2>Appearance</h2><AppIconPicker value={appIcon} change={choice => layout.change(value => ({ ...value, appIcon: choice }))}/><div className="setting-row"><div><strong>Color theme</strong><p>Shared with your other connected clients.</p></div><select aria-label="Color theme" value={currentLayout.theme} onChange={event => layout.change(value => ({ ...value, theme: event.target.value as Layout['theme'] }))}><option value="light">Light</option><option value="dark">Dark</option><option value="system">Follow this device</option></select></div><div className="setting-row"><div><strong>Home timezone</strong><p>The date and clock on your Home board.</p></div><select aria-label="Home timezone" value={currentLayout.timezone} onChange={event => layout.change(value => ({ ...value, timezone: event.target.value }))}>{[...new Set([currentLayout.timezone, Intl.DateTimeFormat().resolvedOptions().timeZone, 'UTC', 'America/Los_Angeles', 'America/New_York', 'Europe/London', 'Asia/Dubai', 'Asia/Tokyo'])].map(zone => <option key={zone}>{zone}</option>)}</select></div><details className="settings-details"><summary>Reset layout</summary><div className="setting-row"><div><strong>Home board</strong><p>Reset the arrangement of the original widgets. Custom widgets, notes, links, tasks and drafts are kept.</p></div><button onClick={() => { setUndoLayout(captureLayoutReset(currentLayout, 'home')); layout.change(value => ({ ...value, widgets: resetHomeWidgets(value.widgets), showCompleted: false })); }}>Reset arrangement</button></div><div className="setting-row"><div><strong>Navigation order</strong><p>Settings always stays pinned at the bottom.</p></div><button onClick={() => { setUndoLayout(captureLayoutReset(currentLayout, 'navigation')); layout.change(value => ({ ...value, nav: [...moduleIds] })); }}>Reset navigation</button></div></details>{undoLayout && <div className="notice"><span>Previous arrangement is available.</span><button onClick={() => { layout.change(value => undoLayoutReset(value, undoLayout)); setUndoLayout(null); }}>Undo reset</button></div>}</section><div className="settings-about"><NovaAppMark choice={appIcon} width="40" height="40"/><div><strong>Nova Dream</strong>Version {__E3_VERSION__}<details><summary>Technical details</summary><p>Internal build: {__E3_BUILD__}</p></details></div></div></>}/></Suspense>}
      {(['contacts', 'content', 'agents', 'profile'] as const).map(page => route === page && <RenderingFallback key={page} fallback={<main className="page-scroll"><Empty title="This module could not load" action={<button onClick={() => location.reload()}>Reload workspace</button>}>Your saved records and kept writing remain available.</Empty></main>}><Suspense fallback={<ModuleLoading module={modules[page].label}/>}>{page === 'contacts' ? <Contacts openSettings={() => openSettings('accounts', 'contacts')} key={`${snapshot.epoch}:${snapshot.deviceId}`} snapshot={snapshot} refresh={refresh} editTask={editTask} target={recordTarget} openEmail={openEmail} openCalendar={() => open('calendar')}/> : <RecordsPage openAssistantSettings={() => openSettings('assistant', page)} openWork={id=>{assistant.select(id,'work');open('assistant');}} openEmail={openEmail} outputs={assistant.outputs} openContent={openContent} openSource={openContentSource} page={page} snapshot={snapshot} refresh={refresh} editTask={editTask} target={recordTarget}/>}</Suspense></RenderingFallback>)}
      {route !== 'assistant' && <VoicePanel appIcon={appIcon} floating controller={voice} openConversation={id => { assistant.select(id); open('assistant'); }}/> }
