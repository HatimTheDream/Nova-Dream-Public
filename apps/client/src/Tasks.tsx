...
export function Tasks({ snapshot, newTask, editTask, changeStatus, refresh, actions, openCalendar, routineTarget, clearRoutineTarget }: Props) {
  ...
  const planDefaults = (): Partial<Task> => ({ projectId: project === 'all' || !project || projectIsDeleted(snapshot, project) ? null : project, planned: view === 'Today' ? today : view === 'Scheduled' ? tomorrow : '', bucket: 'capture', timezone: snapshot.layout.value.timezone });
  ...
  const newRoutine = () => { const next = readLocal<NewRoutineDraft>(`e3:new-routine:${snapshot.deviceId}`) ?? { id: `routine:${crypto.randomUUID()}`, revision: 0, deviceId: snapshot.deviceId, updatedAt: new Date().toISOString(), quickSource: quick, value: { title: quick.title.trim(), notes: '', kind: 'task' as const, state: 'active' as const, startsOn: today, timezone: snapshot.layout.value.timezone, cadence: 'daily' as const, weekdays: [1, 2, 3, 4, 5], projectId: project === 'all' || !project || projectIsDeleted(snapshot, project) ? null : project, plannedTime: '', priority: 'normal' as const, estimateMinutes: 0 } }; saveLocal(`e3:new-routine:${snapshot.deviceId}`, next); setRoutine(next); };
