  ...
  return <Dialog title={widget ? 'Customize Widget' : current ? 'Create A Widget' : 'Add Widgets'} close={leave}>
    <div className="home-widget-dialog">
      ...
      {!current ? <>
        ...
        <div className="home-widget-color-editor"><fieldset className="home-widget-colors"><legend>Color</legend><div>{homeWidgetColors.map(color => <button type="button" key={color} aria-pressed={(current.color ?? 'default') === color} onClick={() => patch({ color })}><span className={`home-color-swatch swatch-${color}`} data-widget-color={color} aria-hidden="true"/><span>{colorNames[color]}</span></button>)}</div><p className="metadata">Default keeps this widget’s original color.</p></fieldset><div className="home-widget-color-preview"><span>Color Preview · Example</span><WidgetPreview widget={{ ...current, size: 'compact' }} sample/></div></div>
        ...
        {(type === 'welcome' || type === 'clock') && <label>Timezone<input value={current.settings?.timezone ?? ''} maxLength={80} placeholder="Use workspace timezone" onChange={event => settings({ timezone: event.target.value || undefined })}/><small>Use an IANA name such as America/Los_Angeles or Europe/London. Leave blank to follow your workspace.</small></label>}
        ...
        <section className="home-widget-live-preview" aria-label="Widget content preview"><div><strong>Preview</strong><small>Sample content · actual size follows your board</small></div><WidgetPreview widget={current}/></section>
