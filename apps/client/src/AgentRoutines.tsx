...
import { routineScheduleFields, routineCron } from './agent-routine-schedule';
...
export function AgentRoutines({ snapshot, openPlan }: { snapshot: Snapshot; openPlan: (id: string) => void }) {
  ...
  return <><div className="record-toolbar"><p className="metadata">Recurring work uses saved assignment plans. This host must be awake and the Assistant connected.</p><button className="primary" onClick={create}><Plus size={18}/>New routine</button></div>
    <div className="record-filters"><label>Show<select value={archived ? 'archived' : 'current'} onChange={e => setArchived(e.target.value === 'archived')}><option value="current">Current routines</option><option value="archived">Archived routines</option></select></label></div>
...
function RoutineEditor({ id, entity, snapshot, reload, close, openPlan }: { id: string; entity?: AgentRoutine; snapshot: Snapshot; reload: () => Promise<unknown>; close: () => void; openPlan: (id: string) => void }) {
  ...
  const scheduleFields = routineScheduleFields(value.schedule);
  if (customSchedule && value.schedule.kind === 'cron') scheduleFields.kind = 'cron';
  ...
  return <section className="card record-editor" aria-label="Routine editor"><div className="section-heading"><div><span className="eyebrow">{entity ? `Routine · version ${entity.revision}` : 'New routine'}</span><h2>{value.name || 'A steady rhythm'}</h2></div><button onClick={close}>Keep & close</button></div>
    ...
    <form onSubmit={e => { e.preventDefault(); void save(); }}><fieldset disabled={busy || !!draft.pending || value.archived}>
      ...
      <div className="record-field-grid"><label>Schedule<select value={scheduleFields.kind} onChange={e => { setCustomSchedule(e.target.value === 'cron'); change({ schedule: e.target.value === 'once' ? { kind: 'once', date: (() => { try { return dayInZone(value.timezone, Date.now() + 86400000); } catch { return ''; } })(), time: '09:00' } : e.target.value === 'every' ? { kind: 'every', minutes: 60, anchor: new Date(Math.ceil((Date.now() + 3600000) / 60000) * 60000).toISOString() } : e.target.value === 'daily' || e.target.value === 'weekly' ? routineCron(scheduleFields.time, e.target.value === 'weekly' ? scheduleFields.weekday : '*') : { kind: 'cron', expression: value.schedule.kind === 'cron' ? value.schedule.expression : '0 9 * * *' } }); }}><option value="daily">Daily</option><option value="weekly">Weekly</option><option value="cron">Custom cron</option><option value="every">Every interval</option><option value="once">One time</option></select></label><label>Timezone<input required value={value.timezone} onChange={e => change({ timezone: e.target.value })}/></label></div>
      {(scheduleFields.kind === 'daily' || scheduleFields.kind === 'weekly') && <div className="record-field-grid">{scheduleFields.kind === 'weekly' && <label>Weekday<select value={scheduleFields.weekday} onChange={e => change({ schedule: routineCron(scheduleFields.time, e.target.value) })}>{['Sunday', 'Monday', 'Tuesday', 'Wednesday', 'Thursday', 'Friday', 'Saturday'].map((day, index) => <option key={day} value={index}>{day}</option>)}</select></label>}<label>Time<input required type="time" value={scheduleFields.time} onChange={e => { if (e.target.value) change({ schedule: routineCron(e.target.value, scheduleFields.kind === 'weekly' ? scheduleFields.weekday : '*') }); }}/></label></div>}
      {value.schedule.kind === 'cron' && scheduleFields.kind === 'cron' && <label>Cron expression<input required maxLength={200} value={value.schedule.expression} onChange={e => change({ schedule: { kind: 'cron', expression: e.target.value } })}/><small className="metadata">Minute · hour · day · month · weekday. For example, 0 9 * * 1-5 runs at 9:00 on weekdays.</small></label>}
