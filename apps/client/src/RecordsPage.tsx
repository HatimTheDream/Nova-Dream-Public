...
type SourceActions = { openWork?: (conversationId: string) => void; openEmail?: (source: MailContactSource) => Promise<void>; outputs: AssistantOutput[]; openContent: (id: string) => void; openSource: (source: ContentOutputSource) => void };
type LocalIndex = { selected: string | null; drafts: string[]; seenTarget?: string };
export default function RecordsPage({ page, snapshot, refresh, editTask, target, outputs, openContent, openSource, openEmail, openWork }: SourceActions & { page: Page; snapshot: Snapshot; refresh: () => Promise<void>; editTask: (task: Entity<Task>) => void; target?: RecordTarget | null }) {
...
function RecordCollection({ kind, snapshot, refresh, editTask, target, outputs, openContent, openSource, openEmail, openWork }: SourceActions & { kind: RecordKind; snapshot: Snapshot; refresh: () => Promise<void>; editTask: (task: Entity<Task>) => void; target?: RecordTarget | null }) {
