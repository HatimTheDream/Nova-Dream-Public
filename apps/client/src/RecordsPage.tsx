...
type SourceActions = { openEmail?: (source: MailContactSource) => Promise<void>; outputs: AssistantOutput[]; openContent: (id: string) => void; openSource: (source: ContentOutputSource) => void };
type LocalIndex = { selected: string | null; drafts: string[]; seenTarget?: string };
export default function RecordsPage({ page, snapshot, refresh, editTask, target, outputs, openContent, openSource, openEmail }: SourceActions & { page: Page; snapshot: Snapshot; refresh: () => Promise<void>; editTask: (task: Entity<Task>) => void; target?: RecordTarget | null }) {
  ...
  return <main ref={scrollRef} onScroll={event => saveLocal(scrollKey, event.currentTarget.scrollTop)} className={`page-scroll records-page records-page-${page}${page === 'agents' && ['agent', 'hub'].includes(kind) ? ' records-hub' : ''}`}>
    ...
    {page === 'agents' && !['agent', 'hub'].includes(kind) && <div className="page-intro"><button onClick={() => setAgentView('agent')}>← Team</button><h1>{kind === 'assignment' ? 'Assignments' : kind === 'routines' ? 'Routines' : 'Skills'}</h1></div>}
    {navigationError && <p role="alert" className="field-error">{navigationError}</p>}
    {kind === 'skills' ? <Suspense fallback={<p role="status">Opening skills…</p>}><AgentSkills key={`${snapshot.epoch}:${snapshot.deviceId}`} snapshot={snapshot}/></Suspense> : page === 'agents' && (kind === 'hub' || kind === 'agent') ? <Suspense fallback={<p role="status">Opening your team…</p>}><AgentHub key={`${snapshot.epoch}:${snapshot.deviceId}`} snapshot={snapshot} presentation={kind === 'hub' ? 'hub' : 'team'} setPresentation={view => setAgentView(view === 'hub' ? 'hub' : 'agent')} openAgent={openDesign} assignWork={assignWork} openPlan={openPlan} openRoutine={openRoutine} openContent={openContent} editTask={editTask} manage={setAgentView} unfinished={agentIndex.drafts.filter(id => !snapshot.records?.agent.some(a => a.id === id))}/></Suspense> : kind === 'routines' ? <AgentRoutines key={`${snapshot.epoch}:${snapshot.deviceId}`} snapshot={snapshot} openPlan={openPlan}/> : kind === 'profile' ? <Suspense fallback={<p role="status">Opening Profile…</p>}><ProfilePage key={`${snapshot.deviceId}:${snapshot.epoch}`} snapshot={snapshot} refresh={refresh} editTask={editTask}/></Suspense> : <RecordCollection key={`${snapshot.deviceId}:${snapshot.epoch}:${kind}`} kind={kind as RecordKind} snapshot={snapshot} refresh={refresh} editTask={editTask} target={target} outputs={outputs} openContent={openContent} openSource={openSource} openEmail={openEmail}/>}
...
function RecordCollection({ kind, snapshot, refresh, editTask, target, outputs, openContent, openSource, openEmail }: SourceActions & { kind: RecordKind; snapshot: Snapshot; refresh: () => Promise<void>; editTask: (task: Entity<Task>) => void; target?: RecordTarget | null }) {
