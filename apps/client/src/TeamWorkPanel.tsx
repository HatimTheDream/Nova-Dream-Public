...
import { suggestedTeamMembers,teamWorkReadiness,teamWorkStatus } from './team-work-state';
...
type Props={snapshot:Snapshot;projectId?:string|null;openConversation:(id:string)=>void;openProjects?:()=>void;openAgents?:()=>void;active?:boolean};
...
function TeamWorkSession({snapshot,projectId,openConversation,openProjects,openAgents,active=true}:Props){
  ...
  const readiness=teamWorkReadiness(form,projects,agents),readinessId=useId();
  ...
  const start=async()=>{
    ...
    if(!retained&&readiness){setError(readiness);return;}
  ...
  return <section className="team-work-panel"><div className="section-heading"><h3>Team work</h3><button type="button" onClick={()=>setEditing(v=>!v)}>{editing?'Back to runs':'New workflow'}</button></div><p className="metadata">Different members, one shared checkout. Each stage receives the earlier handoffs. You review and publish the final changes.</p>
    {(editing||!runs.length)&&<form onSubmit={e=>{e.preventDefault();void start();}}>{!pending&&!pendingAction&&readiness&&<div id={readinessId} className="team-setup-readiness" role="status"><p>{readiness}</p>{!projects.length&&openProjects&&<button type="button" onClick={openProjects}>Add Work Project</button>}{projects.length>0&&agents.length<2&&openAgents&&<button type="button" onClick={openAgents}>Open Agents</button>}</div>}<fieldset disabled={busy||!!pending||!!pendingAction}><label>Work Project<select required value={form.projectId} onChange={e=>change({...form,projectId:e.target.value})}><option value="">Choose a project</option>{form.projectId&&!projects.some(p=>p.id===form.projectId)&&<option value={form.projectId} disabled>Unavailable saved project</option>}{projects.map(p=><option key={p.id} value={p.id}>{p.value.name}</option>)}</select></label><label>Title<input required maxLength={120} value={form.title} onChange={e=>change({...form,title:e.target.value})}/></label><label>What should the team do?<textarea required rows={4} maxLength={15000} value={form.brief} onChange={e=>change({...form,brief:e.target.value})} placeholder="Describe the outcome and how you will know it works."/></label><div className="team-stage-edit">{form.steps.map((step,i)=><label key={i}>{i+1}. {step.role==='research'?'Research & plan':step.role==='build'?'Implement':'Review'}<select required value={step.agentId} onChange={e=>change({...form,steps:form.steps.map((s,n)=>n===i?{...s,agentId:e.target.value}:s)})}><option value="">Choose an agent</option>{step.agentId&&!agents.some(a=>a.id===step.agentId)&&<option value={step.agentId} disabled>Unavailable saved agent</option>}{agents.map(a=><option key={a.id} value={a.id}>{a.value.name} · {a.value.position}</option>)}</select></label>)}</div><details><summary>Time Limit · {form.maxMinutes} Minutes Per Stage</summary><label>Time limit per stage<select value={form.maxMinutes} onChange={e=>change({...form,maxMinutes:Number(e.target.value)})}>{[5,10,20,30].map(minutes=><option key={minutes} value={minutes}>{minutes} minutes</option>)}</select></label></details><p className="metadata">Implementation can edit the checkout. Research and review use read-only sessions. Uses your existing connected AI allowance.</p></fieldset><div className="team-setup-actions"><button className="primary" aria-describedby={!pending&&!pendingAction&&readiness?readinessId:undefined} disabled={busy||!!pendingAction||(!pending&&!!readiness)}>{pending?'Reconcile start':'Start team work'}</button></div></form>}
