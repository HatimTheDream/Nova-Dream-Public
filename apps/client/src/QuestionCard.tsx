  ...
  return <section className={`approval-card request-card question-card${paged ? ' paged-question' : ''}`} aria-label={secret ? 'Secure information request' : 'Question from Nova'}>
    <form onSubmit={event => { event.preventDefault(); if (paged && !last) move(1); else if (complete && editable && ready) void act('resolve'); }} onKeyDown={event => {
      ...
      {live ? <>
        ...
        {paged && <div className="question-navigation" aria-label="Question navigation"><button type="button" className="request-secondary" disabled={!page || busy || sending} onClick={() => move(-1)}>Previous</button>{!last ? <button type="button" className="primary request-primary" disabled={!canNext} onClick={() => move(1)}>Next</button> : !uncertain && !sending && !clock.expired && !readOnly && <button type="submit" className="primary request-primary" disabled={!editable || !ready || !complete}>Send answers</button>}</div>}
