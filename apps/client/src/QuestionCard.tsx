...
export function QuestionCard({ item, epoch, ready, refresh, readOnly = false, compact = false }: { item: AssistantQuestion; epoch: string; ready: boolean; refresh: () => Promise<void>; readOnly?: boolean; compact?: boolean }) {
  ...
  return <section className="approval-card request-card question-card" aria-label={secret ? 'Secure information request' : 'Question from Nova'}>
    <form onSubmit={event => { event.preventDefault(); if (complete && editable && ready) void act('resolve'); }}>
      {item.snapshot.questions.map((q, index) => <fieldset key={q.questionId} disabled={!editable}>
        <legend>{item.snapshot.questions.length > 1 && <span className="question-number">Question {index + 1} of {item.snapshot.questions.length}</span>}{q.question}</legend>
      ...
      </> : <><p className="request-state" role="status">{missing ? 'This request is no longer available. Its outcome cannot be confirmed.' : item.action?.message ?? `Question ${item.snapshot.status}.`}</p><div className="request-actions">{missing && <button type="button" className="primary request-primary" disabled={busy || !ready} onClick={() => void act('check')}>Check status</button>}{!item.dismissed && <button type="button" className="request-secondary" disabled={busy} onClick={() => void act('dismiss')}>Dismiss</button>}</div>{hasDraft && <details className="request-history"><summary>Your kept draft answer</summary><p className="preserve-lines">{Object.values(answers).map(v => v.join(', ')).join('\n')}</p><button type="button" className="request-secondary" onClick={() => void copy()}>Copy draft answer</button></details>}</>}
