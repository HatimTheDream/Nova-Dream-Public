...
export class AssignmentService {
  ...
  start(device: string, raw: unknown, scheduled?: { origin: AssignmentRoutineOrigin; admitted: (attempt: AssignmentAttempt) => void }) {
    ...
    const admitted = this.store.admit(device, input, { type: 'assignment.start', ...input, ...(scheduled ? { routine: scheduled.origin } : {}) }, () => {
      ...
      if (capture.binaryFiles?.some(source => !readableIds.has(source.file.id))) throw new Fault(403, 'assignment_source_access', 'This assignment includes PDF, image or Office sources. Give the agent read access to each source Project or Content before starting.');
      ...
      const prompt = [
        ...
        ...(capture.binaryFiles?.length ? [`SAVED SOURCES AVAILABLE TO READ\n${JSON.stringify(capture.binaryFiles)}\nThese are exact saved file identities, not inspected content. Use nova_read sources.list and sources.read to inspect the relevant files. PDFs support text or rendered image view one page at a time. Use image view for scanned pages, diagrams and charts. DOCX, XLSX and PPTX support text view: page selects an extracted section, worksheet or slide with notes, not Word print pagination. Office visual layout and chart pixels are not supplied; formulas are not recalculated. Read every relevant section and say what you inspected; never claim that filenames are source content.`] : []),
