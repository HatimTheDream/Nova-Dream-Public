...
export class Gateway implements AssistantTransport {
  ...
  constructor(private store: Store, private version = 'development', private createClient: (options: GatewayClientOptions) => Client = options => new GatewayClient(options), private purpose: 'assistant' | 'skill-management' | 'permission-control' | 'response-control' | 'approval-review' | 'question-review' | 'browser-control' = 'assistant') {}
  ...
  start() {
    ...
    this.client = this.createClient({
      url: config.url, token: config.token, clientName: 'gateway-client', clientDisplayName: this.purpose === 'assistant' ? 'Nova Dream — Edition 3' : this.purpose === 'browser-control' ? 'Nova Dream — Host browser' : this.purpose === 'question-review' ? 'Nova Dream — Question review' : this.purpose === 'approval-review' ? 'Nova Dream — Approval review' : this.purpose === 'response-control' ? 'Nova Dream — Response controls' : this.purpose === 'permission-control' ? 'Nova Dream — Access controls' : 'Nova Dream — Skill management', clientVersion: this.version, mode: 'backend', platform: process.platform, role: 'operator', scopes: this.purpose === 'question-review' ? ['operator.read', 'operator.questions'] : this.purpose === 'approval-review' ? ['operator.read', 'operator.approvals'] : this.purpose === 'assistant' ? ['operator.read', 'operator.write'] : ['operator.read', 'operator.admin'], caps: this.purpose === 'approval-review' ? ['approvals'] : this.purpose === 'assistant' ? ['tool-events'] : [], minProtocol: 4, maxProtocol: 4, deviceIdentity: identity, requestTimeoutMs: this.purpose === 'browser-control' ? 45000 : 12000,
      hostDeps: {
      ...
      onConnectError: error => {
        ...
        this.connection = { ...this.connection, state: pairing ? 'pairing' : 'error', pairingRequestId: pairing ? String(details.requestId) : undefined, modelAuthReady: false, message: pairing ? 'Approve this exact Edition 3 device in OpenClaw, then reconnect.' : 'OpenClaw could not authenticate or negotiate this connection. Review its address and sign-in.' };
      },
    });
    this.client.start();
  }
  async request<T = Record<string, unknown>>(method: string, params: unknown): Promise<T> {
    ...
    if (!(this.purpose === 'assistant' ? allowedMethods.has(method) || skillReadMethods.has(method) : this.purpose === 'browser-control' ? method === 'browser.request' : this.purpose === 'question-review' ? questionMethods.has(method) : this.purpose === 'approval-review' ? reviewMethods.has(method) : this.purpose === 'response-control' ? method === 'sessions.patch' : this.purpose === 'permission-control' ? ['sessions.create', 'sessions.patch'].includes(method) : skillManagementMethods.has(method))) throw new Fault(403, 'gateway_method', 'This Gateway operation is not exposed by Edition 3.');
    ...
    if (this.purpose !== 'assistant') {
      ...
      if (!this.connection.grantedScopes.includes(this.purpose === 'question-review' ? (method === 'sessions.messages.subscribe' ? 'operator.read' : 'operator.questions') : this.purpose === 'approval-review' ? (method === 'sessions.messages.subscribe' ? 'operator.read' : 'operator.approvals') : skillReadMethods.has(method) ? 'operator.read' : 'operator.admin')) throw new Fault(403, 'gateway_scope', 'OpenClaw has not granted the required management permission.');
    }
