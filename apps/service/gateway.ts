...
export class Gateway implements AssistantTransport {
  ...
  start() {
    ...
    this.client = this.createClient({
      url: config.url, token: config.token, clientName: 'gateway-client', clientDisplayName: this.purpose === 'assistant' ? 'Nova Dream' : this.purpose === 'browser-control' ? 'Nova Dream — Host browser' : this.purpose === 'question-review' ? 'Nova Dream — Question review' : this.purpose === 'approval-review' ? 'Nova Dream — Approval review' : this.purpose === 'response-control' ? 'Nova Dream — Response controls' : this.purpose === 'permission-control' ? 'Nova Dream — Access controls' : 'Nova Dream — Skill management', clientVersion: this.version, mode: 'backend', platform: process.platform, role: 'operator', scopes: this.purpose === 'question-review' ? ['operator.read', 'operator.questions'] : this.purpose === 'approval-review' ? ['operator.read', 'operator.approvals'] : this.purpose === 'assistant' ? ['operator.read', 'operator.write'] : ['operator.read', 'operator.admin'], caps: this.purpose === 'approval-review' ? ['approvals'] : this.purpose === 'assistant' ? ['tool-events'] : [], minProtocol: 4, maxProtocol: 4, deviceIdentity: identity, requestTimeoutMs: this.purpose === 'browser-control' ? 45000 : 12000,
      hostDeps: {
