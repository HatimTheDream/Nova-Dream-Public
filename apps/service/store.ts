...
export class Store {
  ...
  constructor(directory: string, private now: () => number = Date.now, protectedKey?: Buffer, private reviewOnly = false) {
    ...
    if (schema > 53) { this.db.close(); throw new Error('This workspace needs a newer Edition 3 build.'); }
  ...
  captureBackup(version: string, native: BackupSnapshot['native']): BackupSnapshot {
    return this.transaction(() => {
      ...
      const value: BackupSnapshot = { format: backupFormat, schema: 53, version, id: randomUUID(), createdAt: new Date().toISOString(), epoch: this.epoch, cursor: this.entityCursor, entities, history, receipts, services, files, references, native };
