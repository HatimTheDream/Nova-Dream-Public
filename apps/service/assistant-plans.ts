...
import { taskEffortDemand } from '../../packages/domain/auto-effort.js';
...
export class AssistantPlans {
  ...
  observe(operation: AssistantOperation) {
    ...
    const error = operation.state === 'completed' && state === 'failed' ? pending ? 'A planning question is unresolved. Request changes to continue.' : 'The reply did not include a saved proposal. Request changes to prepare one.' : operation.error;
  ...
  decide(device: string, raw: unknown, amend = false) {
    ...
    const admitted = this.store.admit(device, input, { type: amend ? 'plan.amend' : 'plan.approve', ...input }, () => {
      ...
      const operation: AssistantOperation = { ...source, id, requestId: input.requestId, deviceId: device, conversationRevision: conversation.revision, connectionGeneration: conversation.connectionGeneration, nativeId: conversation.nativeId!, nativeKey: conversation.nativeKey, model: conversation.model, thinking: conversation.thinking, fastMode: conversation.fastMode, autoEffort: undefined, nativeRunId: null, state: 'prepared', input: amend ? ('text' in input && typeof input.text === 'string' ? input.text : '') : `Approve and start: ${version.proposal!.title}`, context: { ...context, digest: hash(context) }, createdAt: at, updatedAt: at, text: '', lastSequence: 0, tools: [], plan: undefined, planSequence: undefined, error: undefined, settledAt: undefined, cancelRequested: undefined, effectiveModel: undefined, nativeTurnId: undefined };
