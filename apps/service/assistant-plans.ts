...
import { researchProgressGuidance } from '../../packages/domain/work-mode.js';
...
import { activeResearchOperation } from './assistant-research-progress.js';
...
export class AssistantPlans {
  ...
  toolPolicy(raw: unknown) {
    ...
    if (input.epoch !== this.store.epoch) throw new Fault(409, 'epoch_changed', 'The workspace changed.');
    if (input.toolName === 'nova_research_progress') {
      try { activeResearchOperation(this.store, this.host, input); return { block: false }; }
      catch { return { block: true, blockReason: 'Report progress only from the original active, approved Chat research run.' }; }
    }
