# Reviewed app-only installer

`install.py` and `recovery.py` implement the concrete Linux delivery procedure used by the external updater. They support an unchanged OpenClaw **2026.9.2** package, unchanged dependency lockfile, and unchanged per-workspace schemas **53/55**, with the selected app on schema **55**. A new engine, dependency, or schema migration requires a separately reviewed driver and rehearsal. The driver never runs package installation, removes prior releases, or prunes backups.
