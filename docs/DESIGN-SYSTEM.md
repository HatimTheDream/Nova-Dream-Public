...
Living visual and interaction reference · baseline: application 1.8.4 · updated 19 September 2026.
...
**Assistant expressions:** `NovaAssistantMark` uses the selected Red or Cream palette in the welcome view and all inline or floating voice panels. Resting artwork reuses the approved 192 px app mark. Listening and speaking use their dedicated 256 px square exports, preserving the same executive identity and CSS corner shape. Voice tiles display at 44 × 44 CSS px; welcome artwork displays at 92 × 92, reduced to 76 × 76 in narrow layouts. Avoid additional frames, status ornaments or decorative motion.

Select listening only for a connected call with detected microphone activity and an unmuted microphone. Select speaking for actual Assistant audio playback in a connected call, including while the microphone is muted. Silence, processing, connecting, preparing, ending, ended and error states use the resting expression. Existing voice text and controls remain the authority for call status. Per-turn expression changes never change the saved App Icon choice, favicon, manifest or operating-system icon. Saved personal agent appearances and the portrait/pixel rendering engines are unchanged; these shared Assistant images do not replace them.
