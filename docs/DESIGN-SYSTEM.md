...
Living visual and interaction reference · baseline: application 1.8.3 · updated 19 September 2026.
...
Compact action controls use a 2 px resting edge that compresses to zero on press; standard controls keep a 2 px pressed edge. Press feedback uses an independent translation, preserving transforms that center or position a control. The borderless Home logo participates with a red accent edge; both Red and Cream artwork keep their full tile dimensions. Button-shaped links share the feedback while preserving native link behavior and user-authored labels.
...
**Reference comparison, 19 September 2026:** the primary Get Started control on [Duolingo's public website](https://www.duolingo.com/) uses a solid 4 px lower edge, 12 px corners and 15 px/700 uppercase text. Its white secondary control uses a bordered face and a smaller separate lower shadow. These are observed web controls, not a universal native-app specification. Nova retains its 16 px action corners, Title Case, Sora typography, gold actions and red identity; the shared tactile feedback is the useful reference. An exact Duolingo visual copy remains a proposal, not the adopted design.
