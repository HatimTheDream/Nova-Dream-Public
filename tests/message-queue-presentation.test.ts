...
const render = (queue: QueuedMessage[], operations: AssistantOperation[] = [], blocked = false, connectionState = 'ready') => {
  const controller = { queue, operations, conversation: { id: 'conversation' }, connection: { state: connectionState } } as AssistantController;
  return renderToStaticMarkup(createElement(MessageQueue, { controller, conversationId: 'conversation', epoch: 'epoch', blocked, copy: () => { throw new Error('Rendering must not copy or dispatch writing.'); } }));
};
...
test('unknown original runs remain visible before earlier items and do not offer dispatch again', () => {
  ...
  assert.ok(markup.indexOf('Previously sent writing') < markup.indexOf('Earlier queue items'));
