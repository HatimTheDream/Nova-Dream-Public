...
import { researchProgress } from '../packages/domain/research-progress.js';
...
test('Chat Research captures preparation, approves a distinct read-only native run, and settles from real run events', async t => {
  ...
  const proposal = { title: 'Heat pump evidence', summary: 'Compare real-world performance and total cost.', steps: ['Read primary sources including field studies and manufacturer specifications for the chosen climate.', 'Reconcile assumptions about climate, installation cost and household energy use.', 'Write a cited report with a comparison and limits of the evidence.'], stepTitles: ['Read primary sources', 'Reconcile assumptions', 'Write a cited report'], assumptions: [], verification: ['Sources support the main conclusions'] };
  ...
  const progress = () => researchProgress({ item: f.service.plans.list()[0], operation: f.get(execution.id), connected: true });
  assert.equal(progress().fraction, null, 'Tool observations alone do not invent an effort estimate');
  f.service.researchProgress.report({ ...policyInput, toolCallId: 'estimate-1', estimate: { expectedRevision: 0, activity: 'Comparing the field-study evidence with the longer technical report.', basis: 'The technical comparison and report need more effort than the first source check.', items: [{ id: 'source-check', title: 'Check the first field-study claim', effort: 2, status: 'complete' }, { id: 'technical-read', title: 'Read the longer technical report', effort: 8, status: 'active' }, { id: 'comparison', title: 'Reconcile performance and cost assumptions', effort: 5, status: 'pending' }, { id: 'final-report', title: 'Write and verify the final report', effort: 3, status: 'pending' }] } });
  assert.equal(progress().fraction, 2 / 18); assert.equal(progress().completedMilestones, 0, 'Concrete work may finish before a broad heading is complete');
  f.transport.emit(execution, 3, 'tool', { name: 'update_plan', phase: 'start', toolCallId: 'milestones', args: { explanation: 'Comparing heat pump efficiency assumptions', plan: proposal.stepTitles.map((step, index) => ({ step, status: index === 0 ? 'completed' : index === 1 ? 'in_progress' : 'pending' })) } });
  assert.equal(progress().fraction, 2 / 18); assert.deepEqual(progress().steps.map(step => step.label), proposal.stepTitles);
  f.transport.emit(execution, 4, 'plan', { plan: [{ step: 'Check a newly found field study', status: 'in_progress' }] });
  assert.equal(progress().fraction, 2 / 18, 'A granular replacement preserves the separately admitted estimate');
  assert.ok(f.store.internalRead<AssistantOperation>(`assistant:operation:${execution.id}`)?.plan?.some(step => step.label === proposal.stepTitles[0] && step.status === 'complete'), 'The retained observation is durable');
  f.transport.emit(execution, 2, 'plan', { plan: proposal.stepTitles.map(step => ({ step, status: 'completed' })) });
  assert.equal(progress().fraction, 2 / 18, 'An out-of-order runtime event cannot advance progress');
  f.transport.emit(execution, 5, 'plan', { plan: proposal.stepTitles.map(step => ({ step, status: 'completed' })) });
  assert.equal(progress().fraction, 2 / 18); assert.equal(progress().state, 'live', 'A fully checked plan is not final report delivery');
  assert.equal(f.store.internalRead<AssistantOperation>(`assistant:operation:${execution.id}`)?.researchEstimate?.revision, 1, 'The admitted estimate is durable across native plan/tool observations');
  f.transport.emit(execution, 6, 'assistant', { text: 'A cited report based on the observed source results.' }); f.transport.emit(execution, 7, 'lifecycle', { phase: 'end' });
